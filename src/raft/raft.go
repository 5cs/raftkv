	...
	if args.Term < rf.currentTerm { // caller is outdated leader!
		...
		if args.PrevLogIndex == 0 {
			...
			if iPrevLogIndex >= 0 {
				...
				reply.Success = true // snapshotted
			}
		} else {
			log.Fatal("Can not happend!")
		}
		...
		if reply.Success {
			...
			for k := N + 1; k <= rf.commitIndex; k++ {
				...
				rf.applyCh <- applyMsg
				if rf.app != nil {
					rf.app.Apply(k, rf.log[rf.Index(k)].Command, false) // apply
				}
			}
		}
	...
	for i := range rf.peers {
		...
		go func(i int) {
			...
			for {
				...
				// resend request vote until ok or discover current leader or new term
		...
		go func(i int) {
			...
			for {
				...
				go func() {
					...
					if rf.Index(rf.nextIndex[i]) < 0 {
						isArgs := &InstallSnapshotArgs{}
						isReply := &InstallSnapshotReply{}
						isArgs.Term = currentTerm
						isArgs.LeaderId = rf.me
						isArgs.LastIncludedIndex = rf.lastExcludedIndex
						isArgs.LastIncludedTerm = rf.lastExcludedTerm
						isArgs.Data = rf.persister.ReadSnapshot()
						rf.sendInstallSnapshot(i, isArgs, isReply)
						if isReply.Term > currentTerm {
							rf.currentTerm = isReply.Term
							rf.state = FOLLOWER
						} else if isReply.Term == currentTerm {
							rf.nextIndex[i] = rf.lastExcludedIndex + 1
						}
						rf.mu.Unlock()
						return
					}
					...
					if rf.nextIndex[i] == 1 {
						...
						iPrevLogIndex := rf.Index(rf.nextIndex[i] - 1)
						args.PrevLogTerm = rf.lastExcludedTerm
						if iPrevLogIndex >= 0 {
							args.PrevLogTerm = rf.log[iPrevLogIndex].Term
						}
						args.PrevLogIndex = iPrevLogIndex + 1 + rf.lastExcludedIndex
					} else {
						log.Fatal("Can not happend!")
					}
							...
							if N > rf.commitIndex && rf.log[rf.Index(N)].Term == rf.currentTerm {
								...
								if has*2 > len(rf.peers) {
									for k := rf.commitIndex + 1; k <= N; k++ {
										...
										rf.applyCh <- applyMsg
										if rf.app != nil {
											rf.app.Apply(k, rf.log[rf.Index(k)].Command, true) // apply
										}
									}
									rf.commitIndex = N
								}
	...
	go func() {
		for {
			...
			if rf.Len() == 0 {
				...
				args.LastLogIndex = rf.lastExcludedIndex
				if iLastLogIndex != -1 {
					last := rf.log[iLastLogIndex]
					args.LastLogTerm = last.Term
					args.LastLogIndex = iLastLogIndex + 1 + rf.lastExcludedIndex
				}
			} else {
				log.Fatal("Can not happend!")
			}
